...
export default function(sequelize) {
  const OldSettings = sequelize.define('OldSettings', {
    ...baseFields,
  ...
  OldSettings.prototype.hasLogoAsset = function() {
  ...
  OldSettings.prototype.getLogoFilename = function() {
    ...
    if (this.logo_url && this.logo_url.includes('/')) {
      DeprecationWarnings.warnDirectUrlStorage('old_settings', 'logo_url', {
        settingsId: this.id,
        logoUrl: this.logo_url,
        location: 'OldSettings.getLogoFilename'
      });
  ...
  OldSettings.prototype.getLogoUrl = function() {
    ...
    if (this.hasLogoAsset()) {
      ...
      if (filename) {
        ...
        return `/api/assets/image/old_settings/${this.id}/${filename}`;
      }
    }
    return null;
  };
  ...
  OldSettings.prototype.getLegacyLogoUrl = function() {
    if (this.logo_url) {
      DeprecationWarnings.warnDirectUrlStorage('old_settings', 'logo_url', {
        settingsId: this.id,
        logoUrl: this.logo_url,
        location: 'OldSettings.getLegacyLogoUrl'
      });
    }
  ...
  OldSettings.prototype.isMaintenanceMode = function() {
    return !!this.maintenance_mode;
  };

  OldSettings.prototype.getStudentsAccessMode = function() {
    return this.students_access || 'all';
  };

  OldSettings.prototype.isStudentsAccessEnabled = function() {
    const mode = this.getStudentsAccessMode();
    return mode === 'all' || mode === 'invite_only' || mode === 'authed_only';
  };

  OldSettings.prototype.getContactInfo = function() {
    return {
