...
import { clog, cerror } from '../lib/colorLogger.js';
...
export function logEnvironmentValidation(results) {
  if (results.errors.length > 0) {
    cerror('❌ Environment Variable Errors:');
    results.errors.forEach(error => cerror(`  - ${error}`));
  }

  if (results.warnings.length > 0) {
    clog('⚠️  Environment Variable Warnings:', 'yellow');
    results.warnings.forEach(warning => clog(`  - ${warning}`, 'yellow'));
  }

  if (results.isValid && results.errors.length === 0 && results.warnings.length === 0) {
    clog('✅ All environment variables validated successfully', 'green');
  }
...
export function validateEnvironmentOnStartup(exitOnError = true) {
  ...
  if (!isValid && exitOnError && process.env.NODE_ENV !== 'development') {
    cerror('🛑 Critical environment variables missing. Server startup aborted.');
    cerror('Please set the required environment variables and restart the server.');
    process.exit(1);
  }
