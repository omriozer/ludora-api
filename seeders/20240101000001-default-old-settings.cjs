...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    // Check if old settings already exist
    const existingSettings = await queryInterface.sequelize.query(
      'SELECT id FROM old_settings LIMIT 1',
      { type: Sequelize.QueryTypes.SELECT }
    );

    if (existingSettings.length > 0) {
      console.log('⏭️  Old settings already exist, skipping seed');
      return;
    }

    await queryInterface.bulkInsert('old_settings', [{
      id: uuidv4(),
    ...
    console.log('✅ Default old settings seeded successfully');
  },

  async down(queryInterface, Sequelize) {
    await queryInterface.bulkDelete('old_settings', null, {});
  }
};