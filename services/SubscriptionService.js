...
class SubscriptionService {
	...
	async createPayplusSubscriptionPage({ planId, userId, userEmail, environment }) {
		try {
				...
				if (customerTokens.length > 0) {
					...
					try {
						...
						if (!response.ok) {
							...
							throw new Error(
								`PayPlus token subscription failed: ${response.status} - ${errorData.message || 'Unknown error'}`
							);
						}
          ...
          custom_invoice_name: user.full_name, //`${plan.name} Subscription`,
          more_info: 'מנוי מתחדש',
					items: [
						{
							name: plan.name,
							quantity: 1,
				...
				if (!response.ok) {
					...
					throw new Error(
						`PayPlus subscription page creation failed: ${response.status} - ${
				...
				// Create pending subscription with PayPlus-specific data
        ...
        payment_page_url: subscriptionPageUrl,
				...
				try {
					...
					console.log(
						'🔄 Continuing with subscription creation - payment page created but without transaction tracking'
					);
				}
					...
					try {
						await EmailService.sendSubscriptionConfirmationEmail({
							...
							subscriptionData: {
								...
								next_billing_date:
									nextPaymentDate || this.calculateNextBillingDate(subscriptionHistory.end_date, plan.billing_period),
							},
						});
			...
			return {
				...
				data: {
					...
					nextBillingDate: isActive
						? this.calculateNextBillingDate(subscription.end_date, subscription.SubscriptionPlan?.billing_period)
						: null,
				},
			};
		} catch (error) {
