...
router.get('/', validateContentSearch, async (req, res) => {
  try {
    ...
    luderror.api('Error in GET /edu-content:', error);
    res.status(500).json({
...
router.post('/',
  ...
  async (req, res) => {
    try {
      ...
      luderror.api('Error in POST /edu-content:', error);
      res.status(500).json({
        error: 'Internal server error',
        message: error.message
      });
...
router.post('/upload',
  ...
  async (req, res) => {
    ...
    try {
      ...
      await transaction.rollback();
      luderror.api('Error in POST /edu-content/upload:', error);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    luderror.api(`Error in GET /edu-content/${req.params.id}:`, error);
    res.status(500).json({
      error: 'Internal server error',
      message: error.message
    });
...
router.get('/:id/file', async (req, res) => {
  try {
    ...
    luderror.api(`Error streaming file for content ${req.params.id}:`, error);
...
router.put('/:id',
  ...
  async (req, res) => {
    ...
    try {
      ...
      await transaction.rollback();
      luderror.api(`Error in PUT /edu-content/${req.params.id}:`, error);
...
router.delete('/:id',
  ...
  async (req, res) => {
    ...
    try {
      ...
      await transaction.rollback();
      luderror.api(`Error in DELETE /edu-content/${req.params.id}:`, error);
...
router.get('/:id/usage', async (req, res) => {
  try {
    ...
    luderror.api(`Error getting usage for content ${req.params.id}:`, error);
    res.status(500).json({
      error: 'Internal server error',
      message: error.message
    });
