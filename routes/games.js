...
async function getGamesWithProducts(userId, userRole) {
  ...
  const gamesWithProductsAndLobbies = await Promise.all(games.map(async (game) => {
    ...
    try {
      ...
      luderror.api(`Error fetching lobbies for game ${game.id}:`, error);
...
router.get('/:gameId/contents', async (req, res) => {
  try {
    ...
    luderror.api(`Error in GET /games/${req.params.gameId}/contents:`, error);
...
router.post('/:gameId/content-use',
  ...
  async (req, res) => {
    ...
    try {
      ...
      luderror.api(`Error in POST /games/${req.params.gameId}/content-use:`, error);
...
router.put('/:gameId/content-use/:useId',
  ...
  async (req, res) => {
    ...
    try {
      ...
      await transaction.rollback();
      luderror.api(`Error in PUT /games/${req.params.gameId}/content-use/${req.params.useId}:`, error);
...
router.delete('/:gameId/content-use/:useId', async (req, res) => {
  ...
  try {
    ...
    await transaction.rollback();
    luderror.api(`Error in DELETE /games/${req.params.gameId}/content-use/${req.params.useId}:`, error);
...
router.get('/:gameId/content-stats', async (req, res) => {
  try {
    ...
    luderror.api(`Error in GET /games/${req.params.gameId}/content-stats:`, error);
    res.status(500).json({
      error: 'Internal server error',
      message: error.message
    });
